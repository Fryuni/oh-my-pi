...
export interface ResolveResumableSessionOptions {
	...
	/** Selects the workspace identifier format for the default local session directory. */
...
export async function resolveResumableSession(
	...
	const localSessionDir = sessionDir ?? computeDefaultSessionDir(
		cwd,
		storage,
		undefined,
		resolvedOptions.workspaceIdentifierMode ?? "path",
	);
