...
describe("Settings", () => {
	...
	describe("workspace identifier", () => {
		it("defaults to path and honors configured Git storage modes", () => {
			expect(Settings.isolated().get("workspace.identifier")).toBe("path");
			expect(Settings.isolated({ "workspace.identifier": "git-remote" }).get("workspace.identifier")).toBe(
				"git-remote",
			);
			expect(Settings.isolated({ "workspace.identifier": "git-root" }).get("workspace.identifier")).toBe("git-root");
		});
	});
